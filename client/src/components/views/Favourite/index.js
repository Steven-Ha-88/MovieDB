...
const UserFavourites = (props) => {
  ...
  const fetchMedia = () => {
    axios
      .post("/api/favourite/getFavouriteMedia", variables)
      .then((response) => {
  ...
  const renderTableBody =
    favouritesMedia &&
    favouritesMedia.map((media, index) => {
      return (
        <WatchList
          key={media.mediaId}
          remove={handleRemove}
          media={media}
          image={`${IMAGE_BASE_URL}${IMAGE_SIZE}/${media.mediaBackdrop}`}
        />
      );
    });
      ...
      return (
        <WatchlistContainer>
          ...
          <div style={{ marginTop: "40px", color: "grey" }}>
            <h5 style={{ color: "grey" }}>
              Get started! Add items to your list by searching for your
              favourite show
            </h5>
          </div>
        </WatchlistContainer>
